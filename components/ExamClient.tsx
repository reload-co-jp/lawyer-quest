...
export const ExamClient: FC<Props> = ({ allQuestions }) => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isCurrent ? "#fff" : answered ? "var(--past)" : "var(--text-3)",
                  cursor: "pointer",
                  borderRadius: "4px",
                  lineHeight: 1,
                }}
        ...
        <div style={{ display: "flex", gap: ".5rem", marginBottom: ".75rem" }}>
          ...
          <button
            ...
            style={{
              ...
              background: "var(--surface)",
              border: "1px solid var(--border)",
              color: currentIndex === questions.length - 1 ? "var(--text-3)" : "var(--text-2)",
              fontSize: ".875rem",
  ...
  return (
    <div style={{ maxWidth: "640px", margin: "0 auto", padding: "0 1rem 3rem" }}>
      {/* スコアカード */}
      <div
        style={{
          background: "var(--surface)",
      ...
      >
        <p style={{ fontSize: ".75rem", color: "var(--text-3)", fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase", marginBottom: ".5rem" }}>
          模擬試験 結果
        </p>
        <p style={{ fontSize: "3.5rem", fontWeight: 700, color: "var(--past)", letterSpacing: "-.03em", margin: ".25rem 0" }}>
          {pct}<span style={{ fontSize: "1.25rem" }}>%</span>
        </p>
        <p style={{ fontSize: "1rem", color: "var(--text-2)", margin: "0 0 .75rem" }}>
          {correct} / {total} 問正解
        </p>
            ...
            return (
              <div key={area}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: ".25rem" }}>
                  <span style={{ fontSize: ".8125rem", color: "var(--text-2)" }}>{area}</span>
                  <span style={{ fontSize: ".8125rem", color: "var(--text-1)", fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>
                ...
                <div style={{ height: "4px", background: "var(--border)", borderRadius: "2px", overflow: "hidden" }}>
                  <div
                    style={{
                      height: "100%",
                      width: `${areaPct}%`,
                      background: areaPct >= 70 ? "var(--success)" : areaPct >= 50 ? "var(--warning)" : "var(--error)",
                      borderRadius: "2px",
                    }}
                  />
                </div>
      ...
      {/* 問題一覧・解説 */}
      <p style={{ fontSize: ".75rem", color: "var(--text-3)", fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase", marginBottom: ".75rem" }}>
        全問解説
      </p>
        ...
        {questions.map((q, i) => {
          ...
          return (
            ...
            >
              ...
              >
                <span style={{ flexShrink: 0, fontWeight: 700, fontSize: ".875rem", color: isCorrect ? "var(--success)" : "var(--error)", minWidth: "1.5rem" }}>
                  {isCorrect ? "○" : "✗"}
                </span>
                <span style={{ fontSize: ".8125rem", color: "var(--text-2)", flex: 1, lineHeight: 1.5 }}>
                  <span style={{ color: "var(--text-3)", marginRight: ".375rem" }}>問{i + 1}.</span>
                  {q.question.length > 60 ? q.question.slice(0, 60) + "…" : q.question}
                </span>
                <span style={{ flexShrink: 0, fontSize: ".6875rem", color: "var(--text-3)" }}>
                  {isOpen ? "▲" : "▼"}
                </span>
              </button>
              ...
              {isOpen && (
                <div style={{ padding: "0 1rem 1rem", borderTop: "1px solid var(--border)" }}>
                  <p style={{ fontSize: ".875rem", color: "var(--text-1)", lineHeight: 1.75, margin: ".875rem 0" }}>
                    {q.question}
                  </p>
                  ...
                  <div style={{ padding: ".75rem", background: "var(--surface-2)", borderLeft: "2px solid var(--accent)", marginBottom: ".5rem" }}>
                    <p style={{ fontSize: ".6875rem", color: "var(--accent)", fontWeight: 600, marginBottom: ".25rem", letterSpacing: ".06em", textTransform: "uppercase" }}>解説</p>
                    <p style={{ fontSize: ".8125rem", color: "var(--text-1)", lineHeight: 1.7, margin: 0 }}>{q.explanation}</p>
                  </div>

                  {q.point && (
                    <div style={{ padding: ".625rem .75rem", background: "var(--surface-2)", borderLeft: "2px solid var(--warning)" }}>
                      ...
                      <p style={{ fontSize: ".8125rem", color: "var(--text-1)", lineHeight: 1.6, margin: 0 }}>{q.point}</p>
                    </div>
                  )}
                </div>
              )}
      ...
      <div style={{ display: "flex", gap: ".5rem" }}>
        <button
          onClick={() => { setPhase("setup"); setReviewIndex(null) }}
          style={{
            flex: 1,
            padding: ".75rem",
            background: "var(--surface)",
